...
class HostelKhojoApp {
  ...
  init() {
    ...
    // Handle URL routing for /admin, /user, /owner or #hostel-{id}
  ...
  handleUrlRouting() {
    ...
    if (hash.startsWith("#hostel-") || hash.startsWith("#/hostel/")) {
  ...
  toggleBookmark(id) {
    ...
    const pageSaveBtn = document.getElementById(`page-save-btn-${id}`);
    if (pageSaveBtn) {
      const isSaved = this.savedIds.includes(id);
      pageSaveBtn.className = `btn btn-outline btn-sm ${isSaved ? 'active' : ''}`;
      pageSaveBtn.innerHTML = `<i class="fa-solid fa-bookmark"></i> ${isSaved ? 'Saved' : 'Save'}`;
    }
  }
  ...
  switchTabMessPane(btnEl, paneId) {
    document.querySelectorAll(".mess-tab-btn").forEach(b => b.classList.remove("active"));
    if (btnEl) btnEl.classList.add("active");
    document.querySelectorAll(".mess-pane-content").forEach(p => p.style.display = "none");
    const target = document.getElementById(paneId);
    if (target) target.style.display = "flex";
  }

  selectReviewStarRating(ratingVal) {
    const hiddenInp = document.getElementById("review-rating-val");
    if (hiddenInp) hiddenInp.value = ratingVal;
    const stars = document.querySelectorAll("#star-rating-picker .star-icon");
  ...
  submitHostelReview(e, hostelId) {
    e.preventDefault();
    const name = document.getElementById("review-author-name")?.value.trim() || (this.currentUser?.full_name || "Student Resident");
    const major = document.getElementById("review-author-major")?.value.trim() || "University Student";
    const comment = document.getElementById("review-comment")?.value.trim();
    const rating = parseFloat(document.getElementById("review-rating-val")?.value) || 5;

    if (!comment) {
      this.showToast("Please enter your review comments.", "warning");
      return;
    }

    let hostel = this.hostels.find(h => String(h.id) === String(hostelId) || (h.name && h.name.toLowerCase() === String(hostelId).toLowerCase()));
    if (hostel) {
      if (!Array.isArray(hostel.reviews)) hostel.reviews = [];
      hostel.reviews.unshift({
        name,
        major,
        rating,
        comment,
